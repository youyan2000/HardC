...
//   互补+死区仍按 sync_rect 使能 — duty_max 0.97 已留裕量 (头注释声明).
...
// 配置单腿定时器 (start 时)
static void bb_config_leg(PwmBuckBoost *me, BspPwmTimer timer, uint32_t mask, float duty, BspPwmTimerConfig *tcfg) {
  uint32_t c1, c3;
...
static void bb_set_deadtime(PwmBase *base, uint32_t deadtime_ns) {
  ...
  if (!me->sync_rect)
    return;
  for (uint8_t i = 0; i < me->num_phases; i++) {
    bsp_update_deadtime(me->bsp_cfg.handle, me->phases[i].timer_a, deadtime_ns, deadtime_ns);
    if (me->phases[i].leg_b_used) {
      bsp_update_deadtime(me->bsp_cfg.handle, me->phases[i].timer_b, deadtime_ns, deadtime_ns);
    }
  }
}
...
void pwm_bb_init_phases(PwmBuckBoost *me, uint32_t freq_hz, uint8_t num_phases, const PwmBbPhaseCfg *cfg, PwmMode mode,
  ...
  if (num_phases == 0u)
    num_phases = 1u;
  if (num_phases > PWM_BB_MAX_PHASES)
    num_phases = PWM_BB_MAX_PHASES;
