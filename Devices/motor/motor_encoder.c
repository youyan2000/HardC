// 位置编码器驱动实现 — motor 域转子位置反馈 (Devices 层)
//
// 来源: TI controlSUITE position_manager
// 翻译为 HardC 纯C 版本
//
// 协议分发: encoder_read_position 按 cfg.proto switch, 具体协议实现各自独立
//   本文件只做帧解析 + 速度估计, 硬件 I/O 由总线指针 (uart/gpio/adc) + rx_buf 注入
//   (总线 IRQ/App 填帧, 本类不直接驱动硬件收发)
//
...
// 构建 Endat 命令帧到 tx 缓冲 (供 encoder_request 使用)
...
void encoder_init(Encoder *me, const EncoderCfg *cfg, Uart *uart, Gpio *gpio, BspAdcHandle *adc) {
  ...
  // 绑定总线指针 (NULL=未绑定该总线, 帧经 rx_buf 注入)
  me->uart = uart;
  me->gpio = gpio;
  me->adc = adc;
...
ErrorCode encoder_request(Encoder *me) {
  ...
  case EncProto_Endat22:
    // Endat: 复用 endat_build_cmd 构建读位置命令帧 → uart_write
    if (me->uart == NULL) {
      return ERR_STATE;  // 串行总线未绑定
    }
    {
      uint8_t tx_buf[4];
      int tx_len = endat_build_cmd(tx_buf, ENDAT_CMD_READ_POS, 0);
      CommConstData data = {tx_buf, (uint16_t) tx_len};
      return uart_write(me->uart, data, IO_NONE);
    }
  case EncProto_BiSS_C:
    // BiSS-C: 需 RS485 方向控制 + MA 脉冲, 超出纯 Uart 能力
    return ERR_NOT_SUPPORT;
  case EncProto_TFormat:
  case EncProto_SinCos:
  case EncProto_PTO:
    // 编码器自主输出 / 模拟采样 / 硬件计数, 无需主机数字请求
    return ERR_OK;
  default:
    return ERR_NOT_SUPPORT;
  }
...
ErrorCode encoder_read_position(Encoder *me) {
  ...
  default:
    return ERR_STATE;  // 未知协议
  }

  if (result != 0) {
    // 错误映射: 原 -1 (帧短/未对齐) → ERR_TIMEOUT, 原 -2 (CRC 失败) → ERR_CHECK
    return (result == -2) ? ERR_CHECK : ERR_TIMEOUT;
  }

  // 成功解析 → 置在线诊断位 (反映已收到有效位置帧, 供 is_ok/状态查询)
  me->connected = true;
  ...
  // 计算角度 (rad)
  me->angle_rad = (float) me->single_turn * M_2PI / (float) ((uint64_t) 1 << me->cfg.bits_single);
