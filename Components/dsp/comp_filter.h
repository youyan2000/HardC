...
// 初始化一阶低通滤波器
static inline void LowPassFilter_Init(LowPassFilter *me, float cut_freq) {
...
static inline float __attribute__((always_inline)) LowPassFilter_Update(LowPassFilter *me, float sample, float dt) {
  // 首帧直接返回原值, 不滤波
  if (!me->initialized_) {
    me->last_out_ = sample;
...
// 重置滤波器状态 (跳变后重新收敛)
static inline void LowPassFilter_Reset(LowPassFilter *me, float sample) {
  me->last_out_ = sample;
}
...
static inline void LowPassFilter2p_Init(LowPassFilter2p *me, float sample_freq, float cutoff_freq) {
...
// 二阶巴特沃斯滤波
static inline float __attribute__((always_inline)) LowPassFilter2p_Update(LowPassFilter2p *me, float sample) {
  // Direct Form I: w = sample - a1*w1 - a2*w2,  out = b0*w + b1*w1 + b2*w2
  float w = sample - me->delay_element_1_ * me->a1_ - me->delay_element_2_ * me->a2_;
  ...
  const float OUT = w * me->b0_ + me->delay_element_1_ * me->b1_ + me->delay_element_2_ * me->b2_;
...
static inline float LowPassFilter2p_Reset(LowPassFilter2p *me, float sample) {
...
static inline float notch_filter_run(NotchVars *me, const NotchCoeff *coeff, float sample) {
  me->in = sample;

  me->out =
      coeff->a1 * me->out1 + coeff->a2 * me->out2 + coeff->b0 * me->in + coeff->b1 * me->in1 + coeff->b2 * me->in2;
...
static inline void notch_coeff_update(float dt, float omega, float c2, float c1, NotchCoeff *coeff) {
  // 连续: H(s) = (s² + ω²) / (s² + 2·c1·ω·s + ω²)
  // 双线性离散化
  (void) c2;  // 预留: 当前实现固定用临界带宽 c2_val = c², 参数未参与计算
  float t = dt;
  float w = omega;
  float w2 = w * w;
...
static inline void iir_filter_design(IirBiquad *me, IirFilterKind kind, IirApprox approx, float fs, float fc, float bw,
                                     float ripple_db) {
  me->x1 = 0.0f;
  me->x2 = 0.0f;
  me->b0 = 1.0f;
  ...
  bool is_band = (kind == IIR_FILTER_BPF || kind == IIR_FILTER_BSF || kind == IIR_FILTER_NOTCH);
...
// 便捷包装 — 低通
static inline void iir_lpf_design(IirBiquad *me, IirApprox approx, float fs, float fc, float ripple_db) {
  iir_filter_design(me, IIR_FILTER_LPF, approx, fs, fc, 0.0f, ripple_db);
}

// 便捷包装 — 高通
static inline void iir_hpf_design(IirBiquad *me, IirApprox approx, float fs, float fc, float ripple_db) {
  iir_filter_design(me, IIR_FILTER_HPF, approx, fs, fc, 0.0f, ripple_db);
}

// 便捷包装 — 切比雪夫专用 (LPF/HPF; 其余 kind 退化为 Q 谐振器)
static inline void iir_cheby_design(IirBiquad *me, IirFilterKind kind, float fs, float fc, float ripple_db) {
...
static inline void iir_biquad_to_bsp_biquad(const IirBiquad *me, BspBiquadInst *bsp) {
  bsp->coeffs[0] = me->b0;
  bsp->coeffs[1] = me->b1;
  bsp->coeffs[2] = me->b2;
  bsp->coeffs[3] = -me->a1;
