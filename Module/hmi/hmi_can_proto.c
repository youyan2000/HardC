...
// 帧布局/功率编码见 hmi_can_proto.h 头注释

#include "hmi_can_proto.h"
#include <string.h>
...
static uint16_t sc_encode_power(float p) {
  float enc = clamp_f(p, HMI_CAN_POWER_MIN_W, HMI_CAN_POWER_MAX_W) * 64.0f + 16384.0f;
  if (enc >= 65536.0f) {
    return 65535u;
  }
  return (uint16_t) enc;
...
void hmi_can_tx_telemetry(HmiCanProto *me, const HmiCanTelemetry *t) {
  ...
  if (me->send) {
    me->send(HMI_CAN_TX_ID, me->tx_buf, 8);
    me->tx_count++;
  }
}
...
void hmi_can_on_frame(HmiCanProto *me, uint32_t id, const uint8_t *data, uint8_t len) {
  if (me == NULL || id != HMI_CAN_RX_ID || data == NULL) {
    return;
  }
...
bool hmi_can_poll(HmiCanProto *me) {
  ...
  while (me->poll(&id, &dlc, data)) {
    hmi_can_on_frame(me, id, data, dlc);
    any = true;
  }
  return any;
}
