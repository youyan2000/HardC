...
#include "hmi_cmd_dispatch.h"  // CmdDispatcher (完整定义)
...
// 端口事件回调 — App 在 board_init 写适配函数: can→hmi_can_send_fn 包装, uart→proto_send_fn 包装, led→output_on/off
...
typedef struct {
  HmiPort can;   // CAN 协议端口 (hmi_can_proto)
  HmiPort uart;  // UART 协议端口 (hmi_serial_proto)
