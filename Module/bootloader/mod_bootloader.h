...
//
// 三上下文分工 (严格遵守 HardC):
//   CTX_HMI  : mod_bootloader_rx() — 收包 (UART RX / CAN 订阅回调 → 入 rx_ring), 非阻塞
//   CTX_MAIN : mod_bootloader_poll() — BackgroundTask 调, 解析协议 + CRC 校验 + Flash 写 (IO_SYNC 允许)
//   board_init: mod_bootloader_should_jump() — 若升级标志有效且 App 有效 → 清标志 → bsp_jump_to_app()
//
...
//   - 每收到一个完整块: 校验块 CRC → 写 Flash (offset = block_id * MOD_BL_MAX_BLOCK_SIZE)
//   - 首块触发整区擦除 (App 区)
//   - 全部块收完 (fw_received >= fw_len) → MOD_BL_VERIFY → 校验 App 区 CRC → 跳转
//
// 组帧状态机 (UART 字节流必需, 处理跨帧重组):
//   MOD_BL_FRAME_SYNC   — 找帧头 0xAA
//   MOD_BL_FRAME_HEADER — 收 len(2) + block(2)
//   MOD_BL_FRAME_DATA   — 收 data
//   MOD_BL_FRAME_CRC    — 收 crc32(4)
...
// 升级协议帧头字节
//   注: _BYTE 后缀规避与组帧状态枚举 MOD_BL_FRAME_HEADER 同名冲突 (同名#define 使枚举失效)
...
typedef enum {
  ...
  MOD_BL_FRAME_DATA,     // 收 data
  MOD_BL_FRAME_CRC,      // 收 crc32(4)
} ModBootloaderFrameState;

// 最大单块数据长度 (含首块前 4 字节固件长度字段)
#define MOD_BL_MAX_BLOCK_SIZE 256u

// 运行时实例
typedef struct {
  ModBootloaderTransport transport;   // 传输 (UART/CAN)
  ModBootloaderCfg cfg;
  ...
  ModBootloaderState st;        // 整体状态

  // 组帧状态机 (UART 字节流)
  ModBootloaderFrameState frame_st;
  uint8_t frame_buf[MOD_BL_MAX_BLOCK_SIZE + 4u + 4u];  // data + crc32 (最大帧体)
