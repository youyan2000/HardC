// 引导加载模块实现 — ModBootloader
//
// 三上下文 (严格遵守 HardC):
//   CTX_HMI  : mod_bootloader_rx — 收包入环, 非阻塞
//   CTX_MAIN : mod_bootloader_poll — 协议解析 + CRC + Flash 写 (IO_SYNC), 跳转
...
#define MOD_BL_HEADER_LEN 5u    // [0xAA][len_hi][len_lo][block_hi][block_lo]
#define MOD_BL_CRC_LEN 4u       // crc32
#define MOD_BL_MAX_BLOCK_SIZE 256u  // 单块数据最大 (含首块前 4 字节固件长度)
...
static uint32_t crc_update(const uint8_t *data, uint32_t len, uint32_t init) {
  ...
  for (uint32_t i = 0u; i < len; i++) {
    crc ^= data[i];
    crc = crc32_calc(&crc, 0u, 0xFFFFFFFFu);  // 简化: 用表项 XOR
  }
  return crc;
}
...
static int write_block(ModBootloader *me, uint16_t block_id,
                       const uint8_t *data, uint16_t len) {
  if (block_id > me->fw_len / MOD_BL_MAX_BLOCK_SIZE) {
    return -1;  // 块号超范围
  }
  uint32_t offset = (uint32_t) block_id * MOD_BL_MAX_BLOCK_SIZE;
...
int mod_bootloader_init(ModBootloader *me, const ModBootloaderTransport *transport,
  ...
  if (me->cfg.db_upgrade_key &&
      comp_database_get(&me->db, me->cfg.db_upgrade_key, &flag, sizeof(flag)) == ERR_OK) {
...
static int process_frame(ModBootloader *me) {
  // 从 rx_ring 读一帧 — 这里简化: 假设上层按帧喂入 (mod_bootloader_rx 收到完整帧)
...
void mod_bootloader_poll(ModBootloader *me) {
  ...
  //   - 写 Flash: offset = block_id * MOD_BL_MAX_BLOCK_SIZE
  ...
  if (me->st == MOD_BL_RECV) {
    // 尝试从环取出完整一帧
    // 帧头
    uint8_t hdr[MOD_BL_HEADER_LEN];
    uint16_t got = ring_read(&me->rx_ring, hdr, MOD_BL_HEADER_LEN);
    if (got < MOD_BL_HEADER_LEN) {
      // 不够一帧头, 退回 (简化: 丢弃已读, 等下一帧 — 真机需组帧状态机)
      return;
    }
    if (hdr[0] != MOD_BL_FRAME_HEADER_BYTE) {
      return;  // 帧头错误, 丢弃 (应重同步)
    }
    ...
    // 读 data + crc
    uint8_t buf[MOD_BL_MAX_BLOCK_SIZE + MOD_BL_CRC_LEN];
    got = ring_read(&me->rx_ring, buf, datalen + MOD_BL_CRC_LEN);
    if (got < datalen + MOD_BL_CRC_LEN) {
      return;  // 数据不全, 等下一 poll
    }
    // 计算 CRC (覆盖 len_hi..data 末尾)
    // 简化: 重新拼 len_hi+len_lo+block_hi+block_lo+data 计算
    uint8_t crc_src[MOD_BL_HEADER_LEN - 1u + MOD_BL_MAX_BLOCK_SIZE];
    crc_src[0] = hdr[1]; crc_src[1] = hdr[2];
    crc_src[2] = hdr[3]; crc_src[3] = hdr[4];
    ...
    if (block == 0u) {
      ...
      me->fw_len = ((uint32_t) buf[0] << 24) | ((uint32_t) buf[1] << 16) |
  ...
  } else if (me->st == MOD_BL_IDLE && me->upgrade_requested) {
    // 进入升级模式
    me->st = MOD_BL_RECV;
  }
}
