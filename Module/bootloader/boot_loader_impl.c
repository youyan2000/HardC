// 引导加载模块实现 (完善版) — BootLoader
//
// 本文件是 boot_loader 的完整实现 (对标 LibXR Database 持久化 + STM32Flash):
//   - 字节流组帧状态机 (UART 必需, 处理跨帧重组): SYNC→HEADER→DATA→CRC
//   - 边收边写: 每块校验 CRC → 写 Flash (offset = block * BOOT_BL_MAX_BLOCK_SIZE)
//   - 首块触发 App 区整区擦除
//   - 全部收完 → VERIFY → 清除升级标志 → 跳转 App
//   - 升级标志用 comp_database (主备双块防断电)
//
// 三上下文 (严格遵守 HardC):
//   CTX_HMI  : boot_loader_rx — 字节/帧入 rx_ring, 非阻塞
//   CTX_MAIN : boot_loader_poll — 组帧 + CRC + Flash 写 (IO_SYNC), 跳转
...
#define BOOT_BL_FRAME_FIXED (2u + 2u + 4u)  // len(2) + block(2) + crc(4), 不含帧头/data
...
static int write_block_data(BootLoader *me, uint16_t block_id, const uint8_t *data, uint16_t len) {
...
//   0 = 尚未收满
//   -1 = 帧头错误需重同步 (SYNC 态)
static int frame_feed(BootLoader *me, uint8_t byte) {
  switch (me->frame_st) {
  case BOOT_BL_FRAME_SYNC:
    if (byte == BOOT_BL_FRAME_HEADER_BYTE) {
      me->frame_len = 0u;
      me->frame_st = BOOT_BL_FRAME_HEADER;
    }
    return 0;

  case BOOT_BL_FRAME_HEADER:
    ...
    if (me->frame_len >= 4u) {  // len_hi, len_lo, block_hi, block_lo
      ...
      if (datalen > BOOT_BL_MAX_BLOCK_SIZE) {
        // 长度非法 → 回到 SYNC 重同步
        me->frame_st = BOOT_BL_FRAME_SYNC;
        return -1;
      }
      me->frame_expect = datalen + 4u;  // 还需 data + crc
      me->frame_st = BOOT_BL_FRAME_DATA;
    }
    return 0;

  case BOOT_BL_FRAME_DATA:
    me->frame_buf[me->frame_len++] = byte;
    // frame_buf[0..3] = len/block, [4..] = data
    if (me->frame_len - 4u >= (me->frame_expect - 4u)) {
      me->frame_st = BOOT_BL_FRAME_CRC;
    }
    return 0;

  case BOOT_BL_FRAME_CRC:
    me->frame_buf[me->frame_len++] = byte;
...
static int process_frame(BootLoader *me) {
  ...
  uint32_t crc_recv =
      ((uint32_t) crc_field[0] << 24) | ((uint32_t) crc_field[1] << 16) | ((uint32_t) crc_field[2] << 8) | crc_field[3];
  ...
  if (block == 0u) {
    ...
    me->fw_len = ((uint32_t) data[0] << 24) | ((uint32_t) data[1] << 16) | ((uint32_t) data[2] << 8) | data[3];
  ...
  // 写块到 Flash (offset = block * BOOT_BL_MAX_BLOCK_SIZE)
...
void boot_loader_poll(BootLoader *me) {
  ...
  // IDLE: 若请求升级则进入 RECV
  if (me->st == BOOT_BL_IDLE && me->upgrade_requested) {
    me->st = BOOT_BL_RECV;
  }

  // RECV: 从 rx_ring 逐字节喂组帧状态机, 收满一帧则处理
  if (me->st == BOOT_BL_RECV) {
    while (me->st == BOOT_BL_RECV) {
      // 读一字节
      uint8_t byte;
