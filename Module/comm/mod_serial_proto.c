...
#define LEN_PID      33   // sizeof(PidTuneFrame)
...
void serial_proto_process(SerialProto *me, CmdDispatcher *cmd) {
  ...
  // ======== 0xFB: PID 批量调参 ========
  case FRAME_PID_TUNE: {
    if (len < sizeof(PidTuneFrame)) {
      me->crc_err_count++;
      break;
    }

    const PidTuneFrame *f = (const PidTuneFrame *)frame_buf;
