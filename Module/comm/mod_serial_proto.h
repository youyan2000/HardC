...
// sizeof(PidTuneFrame) = 1 + 4 + 1 + 6*4 + 2 + 1 = 33 bytes (packed)

// 编译期断言: 确保结构体无 padding
#ifndef __cplusplus
_Static_assert(sizeof(PidTuneFrame) == 33, "PidTuneFrame must be 33 bytes packed");
#endif
...
// 帧处理器回调类型
typedef void (*proto_pid_fn)(const PidTuneFrame *frame);
