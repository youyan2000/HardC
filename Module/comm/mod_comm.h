...
//
// 用法:
//   1. mod_comm_init(&app, c1);                      // 绑定 UART
//   2. mod_comm_on(&app, 0x01, my_handler);          // 注册命令回调
//   3. mod_comm_feed(&app, byte);                    // HAL RX 回调中逐字节喂入
//   4. mod_comm_send(&app, 0x02, data, len);         // 组帧发送
...
// 组帧并通过绑定 UART 发送: [HEAD][CMD][LEN][DATA...][CHK]
