...
//   2. mod_can_init(&can);
//   3. App board_init:
//        mod_can_bind(&can, my_can_send, my_can_poll, my_on_referee);
//      on_referee 回调 → mod_supercap_set_referee_power(&sc, ref->power_limit_w)
//   4. 5ms 周期 (1kHz ISR ÷ 5): mod_can_tx_telemetry(&can, &tel);
...
// ======== CAN 帧 ID ========
#define MOD_CAN_TX_ID 0x051u  // 发送遥测帧
#define MOD_CAN_RX_ID 0x061u  // 接收裁判帧
...
#define MOD_CAN_POWER_MIN_W (-256.0f)
#define MOD_CAN_POWER_MAX_W (768.0f)
...
typedef bool (*mod_can_poll_fn)(uint32_t *id, uint8_t *dlc, uint8_t data[8]);    // 轮询收帧, 有帧填并返回 true
typedef void (*mod_can_send_fn)(uint32_t id, const uint8_t *data, uint8_t dlc);  // 发送一帧
typedef void (*mod_can_on_referee_fn)(const ModCanReferee *ref);                 // 收到有效 0x061 后回调

// 协议上下文
typedef struct {
  mod_can_send_fn send;              // TX 接缝
  mod_can_poll_fn poll;              // RX 轮询接缝
  mod_can_on_referee_fn on_referee;  // 0x061 回调 (App 绑定 → mod_supercap_set_referee_power)
  ModCanReferee referee;             // 最近解析的 0x061 (公开, 只读)
