...
//
// 使用方式:
//   1. 用户创建子类结构体, GenPwr 为第一个成员
//   2. 构造时调用 gen_pwr_init(), 再绑定自己的 Device/Component 指针
//   3. 在 tick 中实现具体控制逻辑 (采样→滤波→PID→PWM)
//   4. 故障检测条件由用户根据硬件定义

#include "gen_powerctrl.h"
#include <string.h>

// ======== 构造 ========

void gen_pwr_init(GenPwr *me, const GenPwr_Param *param) {
...
/*
  ...
  PCTRL_RECOVER:
    ...
    3. 软启完成 → PCTRL_IDLE
*/
void gen_pwr_tick(GenPwr *me, float dt) {
  me->tick_cnt++;
...
void gen_pwr_start(GenPwr *me) {
  if (me->st == PCTRL_IDLE || me->st == PCTRL_RECOVER) {
    me->st = PCTRL_RUN;
  }
}

void gen_pwr_stop(GenPwr *me) {
  // 正常停机流程:
  // 1. 斜坡降输出 (如果子类实现了斜坡逻辑)
  // 2. 封波
  // 3. 回到 IDLE
...
void gen_pwr_emergency_stop(GenPwr *me) {
  // 紧急封波: 立即清零 PWM, 不经过斜坡
...
bool gen_pwr_is_fault(const GenPwr *me) {
  return me->st == PCTRL_FAULT || me->st == PCTRL_FAULT_HOLD;
}

PwrSt gen_pwr_get_state(const GenPwr *me) {
  return me->st;
}
