...
//   pwm/adc 段 (ch_drive/duty 限幅/ADC 通道) 注入 GenBuckCfg 非槽位字段;
...
#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>  // NULL (gen_buck_bind)
...
// gen_buck_state 观察, 不进事件环 (避免 MAIN 调用 start/stop 引入第二生产者竞态)
typedef enum {
  GEN_BUCK_EVT_FAULT_OVP = 1,  // 过压保护触发 (去抖确认, FAST 推)
  GEN_BUCK_EVT_FAULT_OCP = 2,  // 过流保护触发 (去抖确认, FAST 推)
} GenBuckEvt;

#define GEN_BUCK_CMD_SET_VREF 1u  // 命令邮箱: 热调输出电压参考 (周期边界生效)

// ======== Buck 运行时实例 ========
typedef struct {
  PowerStage base;  // [首成员!] 父类 — container_of 下溯入口

  // --- 设备绑定 (gen_buck_bind 注入) ---
...
// 构造: 绑定 ops + 存 cfg + 初始化双环 PidReg4 (设备绑定由 gen_buck_bind 完成)
void gen_buck_init(GenBuck *me, const GenBuckCfg *cfg);

// 绑定设备: 同时填充 PowerStage 基类 pwm/adc 指针 (board_init 中调用)
static inline void gen_buck_bind(GenBuck *me, PwmBuckBoost *pwm_buck, AdcDcSampler *adc) {
...
// 热切换参考: vref / iref (同步批量改, MAIN/启动期用; 运行期热调单值用 gen_buck_set_vref)
void gen_buck_set_ref(GenBuck *me, float vref, float iref);

// MAIN 上下文: 热调输出电压参考 — 经命令邮箱周期边界生效 (无锁安全, 最新命令覆盖)
static inline void gen_buck_set_vref(GenBuck *me, float vref) {
  mailbox_post(&me->cmd, GEN_BUCK_CMD_SET_VREF, vref);
}

// MAIN 上下文: 消费保护事件流 (SPSC 环排空); 返回 false = 无事件
static inline bool gen_buck_evt_pop(GenBuck *me, uint8_t *ev) {
  return ring_pop(&me->evt, ev);
}

// 诊断: evt 环满丢弃计数 (A5 — 环满不阻塞 FAST, 丢弃不静默). 读后由调用方对比快照
static inline uint32_t gen_buck_evt_overflow(const GenBuck *me) {
