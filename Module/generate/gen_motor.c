...
void motapp_tick(MotApp *me) {
  ...
  case MOT_SPD:
    ...
    pid_out = pid_compute(me->pid_vel, (float) me->target_speed, (float) me->current_speed);
    break;
  ...
  case MOT_POS:
    ...
    if (me->pid_pos) {
      pid_out = pid_compute(me->pid_pos, (float) me->target_position, (float) me->current_position);
  ...
  case MOT_SP:
    ...
    if (me->pid_cascade) {
      pid_out = pid_cascade_compute(me->pid_cascade, (float) me->target_position, (float) me->current_position,
...
void motapp_estop(MotApp *me) {
  ...
  // 清零速度环积分, 防止下次启动跳变
  if (me->pid_vel)
    pid_reset(me->pid_vel);
  if (me->pid_pos)
    pid_reset(me->pid_pos);
  if (me->pid_cascade)
    pid_cascade_reset(me->pid_cascade);
}
