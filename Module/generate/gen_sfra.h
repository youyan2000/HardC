...
//
// 典型 ISR 调用:
//   gen_sfra_inject(&sfra);
//   ref += sfra.inject_out;      // 扰动叠加到参考点
//   ... 控制算法 ...
//   gen_sfra_collect(&sfra, output);
//
// 主循环:
//   gen_sfra_background(&sfra);   // 频点切换 + DFT 结算 + 结果输出
...
typedef struct {
  ...
  void (*on_point_done)(void *user, int idx, int total, float freq, float gain_db, float phase_deg);
...
void gen_sfra_init(Sfra *me, float dt, const SfraCfg *cfg);

// 重置: 回到空闲状态, 保留 cfg/dt/回调
void gen_sfra_reset(Sfra *me);

// ======== 扫频控制 ========

// 启动扫频: 计算频点列表 → 从 f_start 开始 → settling
void gen_sfra_start(Sfra *me);

// 中止扫频: 立即停止, sweep_done=false
void gen_sfra_stop(Sfra *me);

// ======== ISR 调用 (快速路径, 禁止 printf) ========

// 扰动注入 — 在控制算法之前调用, 输出注入值到 me->inject_out
// 用户将 me->inject_out 叠加到控制环参考点
void gen_sfra_inject(Sfra *me);

// 响应采集 — 在控制算法之后调用
//   response: 被测输出变量 (如 Vout, Iout, 编码器速度等)
void gen_sfra_collect(Sfra *me, float response);

// ======== 主循环调用 (慢速路径, 可 printf) ========

// 后台处理 — 频点切换、DFT 结算、回调通知
void gen_sfra_background(Sfra *me);
