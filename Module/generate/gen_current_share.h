...
//
// 职责 (每 FAST tick, 与 gen_supercap 同源频率):
...
//   gen_share_set_voltages(va, vb) → set_paside(paside) → set_currents(i[3]) → tick()
//
// 与 gen_supercap 分工: supercap 做功率环 + 保护, 本模块做电流均衡 + 模式选择.
// supercap 每个 FAST tick 先注入电压/电流/相电流指令, 再调本模块 tick 写 PWM.
// 急停时 supercap 调 gen_share_emergency → 本模块置 fault, 后续 tick 不再写 PWM.

#ifndef GEN_CURRENT_SHARE_H
#define GEN_CURRENT_SHARE_H
...
#define GEN_SHARE_MAX_PHASES 3  // 最大并联相数 (1..3)

// 电流 PI 输出限幅 (alpha 修正量 u ∈ ±5%, alpha ∈ [0.95, 1.05])
#define GEN_SHARE_ALPHA_DELTA_MAX 0.05f
...
typedef struct {
  ...
  PwmBuckBoost *pwm;  // 输出设备 (gen_share_bind 注入)
  ...
  float i_phase[GEN_SHARE_MAX_PHASES];  // 各相电流 (supercap 注入)
  GenSharePhase ph[GEN_SHARE_MAX_PHASES];
