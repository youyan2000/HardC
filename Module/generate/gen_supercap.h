...
//   均流注入 (gen_current_share, PST_RUN 内):
//     gen_share_set_voltages/paside/currents → gen_share_tick (写 PwmBuckBoost)
//
...
// 绑定顺序: board_init 必须先 gen_share_bind(share, pwm) 再 gen_supercap_bind(...),
...
//   级联绕过 base.loop[2] (PowerStage 仅 2 槽): per-phase 电流 PI 归 gen_current_share
//
...
#ifndef GEN_SUPERCAP_H
#define GEN_SUPERCAP_H
...
#define GEN_SC_MAX_PHASES GEN_SHARE_MAX_PHASES  // 复用均流模块相数上限 (3)
...
typedef struct {
  ...
  float share_gain;       // [9] 均流增益 (注入 gen_current_share)
  ...
  uint8_t adc_ch_i[GEN_SC_MAX_PHASES];  // 相电流 ADC 通道
...
typedef enum {
  GEN_SC_EVT_FAULT_SHORT = ERROR_SHORT_CIRCUIT,        // 0x08 短路
  GEN_SC_EVT_FAULT_UNBALANCE = ERROR_PHASE_UNBALANCE,  // 0x40 三相失平衡
} GenSuperCapEvt;

#define GEN_SC_CMD_SET_REFEREE_POWER 1u  // 裁判功率目标 (W, 正=充电/负=放电, CAN 0x061 喂)

// ======== 超级电容运行时实例 ========
typedef struct {
  PowerStage base;  // [首成员!] 父类 — container_of 下溯入口

  // --- 设备绑定 (gen_supercap_bind 注入) ---
  AdcDcSampler *adc;       // ADC 采样
  GenCurrentShare *share;  // 均流模块 (共享 PwmBuckBoost, supercap 驱动其 tick)
  ...
  float i_phase[GEN_SC_MAX_PHASES];  // 相电流 (A)
...
static inline void gen_supercap_set_referee_power(GenSuperCap *me, float w) {
  mailbox_post(&me->cmd, GEN_SC_CMD_SET_REFEREE_POWER, w);
}

// MAIN 上下文: 消费保护事件流 (SPSC 环排空); 返回 false = 无事件
static inline bool gen_supercap_evt_pop(GenSuperCap *me, uint8_t *ev) {
  return ring_pop(&me->evt, ev);
}

// 诊断: evt 环满丢弃计数 (A5 — 环满不阻塞 FAST, 丢弃不静默). 读后由调用方对比快照
static inline uint32_t gen_supercap_evt_overflow(const GenSuperCap *me) {
