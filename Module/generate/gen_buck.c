...
#define GEN_BUCK_DT 0.0001f

// 故障去抖确认次数 (OVP/OCP 连续 N tick 触发才急停, 防噪声误报)
#define GEN_BUCK_FAULT_DEBOUNCE_N 8u

// ======== 内部辅助 ========

// 采样: AdcDcSampler 工程量 (k/b 校准已在采样器内部完成)
static inline void buck_sample(GenBuck *me) {
...
static inline float buck_soft_start(GenBuck *me, float target) {
...
static bool buck_protect(GenBuck *me) {
  ...
  if (me->base.debounce_cnt < GEN_BUCK_FAULT_DEBOUNCE_N) {
    me->base.debounce_cnt++;
  }
  if (me->base.debounce_cnt >= GEN_BUCK_FAULT_DEBOUNCE_N && !me->fault_latched) {
    ...
    if (me->vout > me->base.ovp) {
      if (!ring_push(&me->evt, GEN_BUCK_EVT_FAULT_OVP)) {
        me->evt_overflow_cnt++;
      }
    }
    if (me->iout > me->base.ocp) {
      if (!ring_push(&me->evt, GEN_BUCK_EVT_FAULT_OCP)) {
        me->evt_overflow_cnt++;
      }
    }
  }
...
static void buck_run(GenBuck *me) {
  // 设备未绑定则拒绝运行
...
static void buck_init(PowerStage *base) {
  GenBuck *me = container_of(base, GenBuck, base);
...
static void buck_tick(PowerStage *base) {
  ...
  if (mailbox_poll(&me->cmd, &c, &a)) {
    if (c == GEN_BUCK_CMD_SET_VREF) {
      me->cfg.vref = a;
      gen_buck_sync_cfg(me);
    }
  }
...
static void buck_stop(PowerStage *base) {
  ...
  me->base.st = PST_IDLE;
  // 同 buck_start: START/STOP 状态变化由调用方读 gen_buck_state 观察, 不推事件环
}
...
static void buck_set_ref(PowerStage *base, float vref, float iref) {
  // 同步批量改参考 (MAIN, 启动/配置期); 运行期热调单值走 gen_buck_set_vref (命令邮箱周期边界生效)
  GenBuck *me = container_of(base, GenBuck, base);
  me->cfg.vref = vref;
  me->cfg.iref = iref;
  gen_buck_sync_cfg(me);
...
void gen_buck_init(GenBuck *me, const GenBuckCfg *cfg) {
  ...
  pid_reg4_init(&me->pid_v, GEN_BUCK_DT, 0.0f, me->base.iref, &r4);
  pid_reg4_init(&me->pid_i, GEN_BUCK_DT, 0.0f, 1.0f, &r4);
  // 挂到 PowerStage.loop 槽位 (PidBase*, 供框架访问)
  me->base.loop[0] = &me->pid_v.base;  // 外环: 电压 → 电流指令
  me->base.loop[1] = &me->pid_i.base;  // 内环: 电流 → 占空比
  gen_buck_sync_cfg(me);               // 由 cfg 派生双环 PI 运行时参数
}

// ======== 配置同步 ========

void gen_buck_sync_cfg(GenBuck *me) {
